...
/**
 * 多边形区域查询
 * 地图上画出一个多边形，然后计算出这个多边形里面有哪些我们所需要的热点，比如查询哪块区域有多少7.11便利店
 * @author 大象
 *
 */
public class WithinQuery {
	...
	public Set<QueryMatch> query(Geometry query) throws IOException {
		...
		for (GeoHash prefix : prefixes) {
			...
			for (Result r : scanner) {
				...
				ret.add(new QueryMatch(id, hash, Double.parseDouble(lon),
		...
		System.out.println("Geometry predicate filtered " + exclusionCount
				+ " points.");
		return ret;
	}
	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		if (args.length != 2
				|| (!"local".equals(args[0]) && !"remote".equals(args[0]))) {
			System.out.println("输入的参数不精确");
			System.exit(0);
		}
